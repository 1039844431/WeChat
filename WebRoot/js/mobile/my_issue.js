...
var my_collect = '<div class="list_container"> '+
				 	'    <div class="book"> '+
				 	 '   		<div class="clip"></div>'+
				 	'    </div>'+
					...
					' 		<div class="company_message_common"><div class="collect_time">发布时间:<font style="color: #EE7621;">COLLECTTIME</font></div></div>'+
					...
					'		<div class="company_message_common"><div class="company_adress">状&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;态：<font style="color: #EE7621;">STATE</font></div></div>'+
					'	</div>'+
					' <a>'+
				...
				$(data.appDetailFromList).each(function (i,category){
					var collect_div = my_collect.replace("APPTITLE", category.appName)
							 ...
							 if(category.isIssue == "0"){								
								collect_div = collect_div.replace("STATE", "审核中");							
							}else{								
						        collect_div = collect_div.replace("STATE", "审核通过");							
							}
...
function initailCollect(){
	...
	setTimeout(function(){
		...
		$.ajax({  
	        url :"system/AppDetailAction_deleteAppdetail.do", 
	        type:"post",      
	        data:{
	        	deleteID:collectID
	        },
