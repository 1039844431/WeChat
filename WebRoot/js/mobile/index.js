...
function initalIndex(){
		...
		addEveryTopic("1","add_yinying_div");
		addEveryTopic("12","add_shejiao_div");
		addEveryTopic("5","add_jinrong_div");
		addEveryTopic("7","add_shiyong_div");
}

function addEveryTopic(number,className){
$.ajax({
		url:"system/AppDetailAction_findCategoryAppDetail.do",
		type:"post",
		dataType:"json",
		data:{CategoryKey:number},
		success:function(data){
			$(data.appDetailFromList).each(function (i,hotRecommend){
				
				$("." + className).after(addGoodAndHotRecommend.replace("APPIMG", hotRecommend.appImg)
						                                       .replace("APPNAME", hotRecommend.appName)
						                                       .replace("APPDETAILID", hotRecommend.ID)
						                                       .replace("APPCATE", hotRecommend.category)
						                                       .replace("APPINFO", hotRecommend.intro)
